...
Trial = 2e6 ; % 2e6 ; 
                                            XR = 10 ; % for indexing self-isolation parameter
...
SymptomPeriod = 1e3 ; % each infected agent shows the symptoms after this period of time
IsolationPeriod = 1.5 * LifeTimeVirus ;  % time period that the agent stays in isolatoin
...
Self_Iso = zeros(XR, 1 ) ; % chance of self-isolations
...
for bv = 1 :       XR    
    CC = 0 ;
    bv
    Self_Iso(bv) = (bv-1) * 0.1 ;  
    ChanceSelf_Iso = Self_Iso(bv) ;   
        ...
        for ti = 2 : Trial
           ...
           % picking agents to play
            ...
            while r1 > P1 || r2 > P2          || Isolated(m) == 1  || Died(m) == 1 || Isolated(n)==1 || Died(n)==1
                ...
                % update time duration of infection and isolation for agens
                if Infected(m) == 1
                ...
                Suum1 = 0 ;
                for to = 1 : Size
                    Suum1 = Suum1 + Chance1( m , to  )  ;   %%%%  + Chance1(  m , to - 1 )  ;
                end
                P1 = Chance1( m , n ) /  Suum1  ;

                Suum2 = 0 ;
                for to = 1 : Size
                    Suum2 = Suum2 + Chance1( n , to  )  ;   %%%%  + Chance1(  m , to - 1 )  ;
                end
                P2 = Chance1( n , m ) /  Suum2  ;
            ...
            Delay(ti, bv) = tiDelay  ;  % number of attempts to pair
            ...
            Suum2 = 0 ;
            for to = 1 : Size
                Suum2 = Suum2 + Chance1( 1 , to ) ;   %%%%  + Chance1(  m , to - 1 )  ;
            end
           ...
           %   updating the tendencies of to whom to play decision SAC
            ...
            % accumulative payoff
        if ti >= TimeVirus  && ti <= TimeVirus +  2 * IsolationPeriod  %  1e5

            AccumPay(ti, bv) =   AccumPay(ti-1, bv)  +  ( Out2( m ) + Out2( n ) )/(2 * Delay(ti, bv) ) ;
        end
            ...
            if  ti > TimeVirus
                ...
                if  Vaccinated(m) == 0 && Infected(n) == 1 
                ...
                if  Vaccinated(n) == 0 && Infected(m) == 1 
                    r = rand ;
                    if r < InfectionChance
                        Infected(n) = 1 ;
                    end
                ...
                for yy11 = 1: Size
                    if    TimeInfected(yy11) > LifeTimeVirus   %%%%% rand <   (  exp(TimeInfected(yy11) - LifeTimeVirus )  )   %%%%
                        ...
                        if r < DeadChance
                            ...
                            % self-isolation (detecting sympthoms in other)

            if  Infected(m) == 1 &&  Died(n) == 0  && Vaccinated(n) == 0  &&  TimeInfected(m) >  SymptomPeriod

                r = rand ;
                if r < ChanceSelf_Iso  % OthersSympt
                    Isolated( n ) = 1 ;
                end
            end


            if  Infected(n) == 1 &&  Died(m) == 0  && Vaccinated(m) == 0 &&  TimeInfected(n) >  SymptomPeriod
                ...
                if r < ChanceSelf_Iso % OthersSympt
                    Isolated( m ) = 1 ;
                            ...
                            % self-isolation (detecting sympthoms in self)

            if  Infected(m) == 1 &&  Died(m) == 0  && Vaccinated(m) == 0 &&  TimeInfected(m) >  SymptomPeriod   %% &&  rand < TimeInfected(m) / LifeTimeVirus  %%%% rand <   (  exp(TimeInfected(m) - LifeTimeVirus )  )

                r = rand ;
                if r < ChanceSelf_Iso % SelfSympt
                    Isolated( m ) = 1 ;
                end
            end


            if Infected(n) == 1 &&  Died(n) == 0  &&  Vaccinated(n) == 0  && TimeInfected(n) >  SymptomPeriod  %%&& rand < TimeInfected(n) / LifeTimeVirus  %%%  rand <   (  exp(TimeInfected(n) - LifeTimeVirus )  )
